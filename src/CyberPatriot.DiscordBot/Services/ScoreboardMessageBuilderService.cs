...
#pragma warning restore 0618
        ...
        public string CreateTopLeaderboardEmbed(CompleteScoreboardSummary scoreboard, TimeZoneInfo timeZone = null, int pageNumber = 1, int pageSize = 15)
        {
            ...
            Console.Out.Write("Division: ");
            Console.Out.WriteLine(scoreboard.Filter.Division);
            Console.Out.Write("Location: ");
            Console.Out.WriteLine(scoreboard.Filter.Location);
            Console.Out.WriteLine((scoreboard.TeamList.Count));
            Console.Out.WriteLine(pageNumber);
            if (--pageNumber < 0 || pageNumber >= pageCount)
            {
                throw new ArgumentOutOfRangeException(nameof(pageNumber));
            }
